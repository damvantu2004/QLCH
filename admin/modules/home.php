...
$sql_sales = "SELECT * FROM orders WHERE order_date BETWEEN '$subdays' AND '$now' AND order_status IN (3, 5)";
...
while ($val = mysqli_fetch_array($query_metric)) {
    if ($val['order_type'] == 6) {
        $order_live++;
    } elseif ($val['order_type'] == 5 || $val['order_type'] == 1 || $val['order_type'] == 2 || $val['order_type'] == 3 || $val['order_type'] == 4) {
        $order_online++;
    }
...
<script>
    $(document).ready(function() {
        ...
        var donut = new Morris.Donut({
            ...
            colors: ['#FABA66', '#76C1FA', '#F36368'] // Màu sắc phù hợp cho nền trắng
        });
